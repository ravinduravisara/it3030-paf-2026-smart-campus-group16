...
public class AuthService {
	...
	public AuthResponse resendOtp(ResendOtpRequest request) {
		...
		if (isVerified(user)) {
			return new AuthResponse(null, null, displayNameFor(user), normalizedRole(user), email, displayNameFor(user), "This account is already verified.");
		}

		String otpMessage = issueOtpFor(user);
		return new AuthResponse(null, null, displayNameFor(user), normalizedRole(user), email, displayNameFor(user), otpMessage);
	}
